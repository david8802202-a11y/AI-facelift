...
import json
import requests
import re # 引入正規表達式來清洗文字

# --- 1. 設定頁面 ---
st.set_page_config(page_title="PTT/Dcard 文案產生器 (V45 邏輯修復版)", page_icon="🛠️")

api_key = st.secrets.get("GOOGLE_API_KEY")

st.title("🛠️ PTT/Dcard 文案產生器 (V45 邏輯修復版)")
...
with col1:
    ...
    # --- 關鍵修正：自動清洗變數，把 Emoji 去掉，避免 AI 照抄 ---
    clean_topic_text = topic_category.split('(')[0]
    # 使用正規表達式去掉所有非文字符號 (Emoji)
...
if st.button("🚀 生成 5 個標題", use_container_width=True):
    ...
    with st.spinner(f"AI 正在邏輯思考中..."):
        try:
            ...
            【嚴格格式要求】：
            1. 標題**必須**以「{target_tag}」開頭 (例如：[討論]...)。
            2. **禁止**使用 Emoji (❌ 💉, ⚡, 🏥)。
            3. **禁止**抄襲上方「參考範例」的內容 (不要寫韓版電波，除非主題真的是韓版電波)。
            4. 字數 16~25 字。
            5. 語氣：{tone_intensity}。
            ...
            # 二次清洗：萬一 AI 還是加了 Emoji 或編號，手動去掉
            clean_titles = []
            for t in titles:
                t = t.strip()
                # 去掉開頭的 "1. " 或 "- "
