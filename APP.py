...
# --- 1. 頁面設定 ---
st.set_page_config(page_title="PTT 文案產生器 V57", page_icon="🎯")
...
st.markdown("---")
imported = st.text_area("📝 匯入網友議題 (若有貼入，AI會優先改寫此內容)：", height=100)
...
if 'sel' in st.session_state:
    ...
    if st.button("✍️ 撰寫完整內文與推文"):
        with st.spinner("正在對齊範本語氣..."):
            info = DB[cat]
            prompt = f"""你是一個 PTT 鄉民。
            模仿這種口吻："{info['example_body']}"
            常用關鍵字：{info['keywords']}
            
            現在請針對「{st.session_state.sel}」寫一篇 120 字的文章。
            要求：第一人稱，禁止開頭問候，內容鎖定標題。
            並附上 8 則回文，格式為「推|內容」。回文要酸、禁止問號。
            """
